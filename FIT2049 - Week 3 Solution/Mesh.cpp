...
Mesh* Mesh::GetMesh(const char* identifier)
{
	...
	{
		// Found the mesh now give me the value
...
Mesh* Mesh::CreateTriangle(Direct3D* direct3D, const char* identifier)
{
	...
	// The order we pass these in matters
	// It tells the renderer whether it is the back or the front
	indices[0] = 0;
	indices[1] = 1;
	indices[2] = 2;

	return new Mesh(direct3D, identifier, vertexCount, indexCount, verts, indices, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

}

Mesh* Mesh::CreateSquare2D(Direct3D* direct3D, const char* identifier)
{
	Vertex* verts;	
	...
	verts[3].colour = Color(1.0f, 0.6f, 0.7f, 1.0f);
	...
	return new Mesh(direct3D, identifier, vertexCount, indexCount, verts, indices, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
}

Mesh* Mesh::CreateAbstractArt(Direct3D* direct3D, const char* identifier)
{
	Vertex* verts;
	unsigned long* indices;
	...
	for (int i = -5; i < 5; i++) {
		for (int j = -5; j < 5; j++) {
			verts[myCounter].position = Vector3((float)j, (float)i, 0.0f);
			verts[myCounter].colour = Color(1.0f, 0.6f, 0.7f, 1.0f);
			myCounter++;
		}
	}

	
	// Here we willspecify the indices
	indices[0] = 0;
	indices[1] = 10;
	indices[2] = 11;
...
void Mesh::Render(Direct3D* direct3D, Matrix world, Camera* cam, Shader* shader, Texture* texture)
{
	...
	shader->SetMatrices(direct3D->GetDeviceContext(), world, cam->GetView(), cam->GetProjection());	//The shader needs to know the world matirx to use to draw the
...
bool Mesh::LoadFromFile(LPCWSTR filename, const char* identifier, Direct3D* direct3D)
{
	...
	meshMap[identifier] = new Mesh(direct3D, identifier, finalVertexCount, indexCount, vertexData, indexData, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
